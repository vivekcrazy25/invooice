...
contextBridge.exposeInMainWorld('db', {
  ...
  inventory: {
    ...
    bulkAdd:      (products) => invoke('bulk-add-products', products),
    showOpenDialog:(opts)    => invoke('show-open-dialog', opts),
    importExcelFile:(path)   => invoke('import-excel-file', path),
  },
  ...
  settings: {
    ...
    toggleUserStatus:      (id, status)    => invoke('toggle-user-status', { id, status }),
    ...
    login:                 (email, password) => invoke('login-user', { email, password }),
    getUserPermissions:    (userId)        => invoke('get-user-permissions', userId),
    updateUserPermissions: (userId, perms) => invoke('update-user-permissions', { userId, permissions: perms }),
    getNotifications:      ()              => invoke('get-notifications'),
    globalSearch:          (q)             => invoke('global-search', q),
  },
});
